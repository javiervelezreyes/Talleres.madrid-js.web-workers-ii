...
let shared = Shared.create ({
  ...
  monitor : Monitor ({
    ...
    async endRead () {
      ...
      this.reading = OFF
           if (this.nReaders > this.nWriters && !isEmpty (buffer)) await readers.signal ()
      else if (this.nWriters > this.nReaders)                      await writers.signal ()
    ...
    async endWrite () {
      ...
      this.writing = OFF
      this.nWriters--
           if (this.nWriters > this.nReaders && !isFull (buffer)) await writers.signal ()
      else if (this.nReaders > this.nWriters)                     await readers.signal ()
